...
public class FrameAnimImageView extends ImageView
{
    ...
    private class AnimTimerTask extends TimerTask
    {
        ...
        public void run()
        {
            if (mFrameIndex < 0 || mState == STATE_STOP || mResourceIdList == null)
            {
                return;
            }

            if (mResourceIdList != null && mFrameIndex < mResourceIdList.size())
            {
    ...
    private Handler AnimHanlder = new Handler()
    {
        public void handleMessage(Message msg)
        {
            ...
            {
                case MSG_START:
                    if (mFrameIndex >= 0 && mResourceIdList != null
                            && mFrameIndex < mResourceIdList.size() && mState == STATE_RUNNING)
                    {
